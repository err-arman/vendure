...
export function SearchControls({ categories }: SearchControlsProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {showPopover && (
        <div className="absolute left-0 right-0 top-full z-40 mt-2 max-h-80 overflow-y-auto rounded-2xl border border-stone-200/70 bg-white/95 p-1.5 shadow-xl shadow-stone-900/[0.08] backdrop-blur-xl sm:left-0 sm:right-auto sm:w-96">
          <button
            type="button"
            onClick={() => navigateToSearch(query.trim())}
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm text-stone-600 transition-colors hover:bg-stone-100"
          >
          ...
          {suggestions.length === 0 ? (
            <p className="px-3 py-2 text-sm text-stone-500">{t("noSearchResultsShort")}</p>
          ) : (
            <ul className="mt-0.5">
                ...
                <li key={item.productId}>
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors ${
                      highlightIndex === index ? "bg-stone-100" : ""
                    }`}
                  >
                    ...
                    <span className="flex flex-1 flex-col">
                      ...
                      <span className="text-xs text-stone-500">
                        {item.priceWithTax?.__typename === "SinglePrice" ? (
                          <Price value={item.priceWithTax.value ?? 0} currencyCode={item.currencyCode ?? undefined} />
                        ) : item.priceWithTax?.__typename === "PriceRange" ? (
                          <Price value={item.priceWithTax.min ?? 0} currencyCode={item.currencyCode ?? undefined} />
                        ) : null}
                      </span>
                    </span>
                  </button>
